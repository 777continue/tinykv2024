...
func (server *Server) KvPrewrite(_ context.Context, req *kvrpcpb.PrewriteRequest) (*kvrpcpb.PrewriteResponse, error) {
	...
	for _, mdf := range req.Mutations {
		write, ts, err := txn.MostRecentWrite(mdf.Key)
		if err != nil {
			if regionErr, ok := err.(*raft_storage.RegionError); ok {
				resp.RegionError = regionErr.RequestErr
				return resp, nil
		...
		case kvrpcpb.Op_Put:
			...
			txn.PutValue(mdf.Key, mdf.Value)
		case kvrpcpb.Op_Del:
			kind = mvcc.WriteKindDelete
			txn.DeleteValue(mdf.Key)
		default:
			return nil, nil
		}
		txn.PutLock(mdf.Key, &mvcc.Lock{
			Primary: req.PrimaryLock,
			Ts:      req.StartVersion,
			Ttl:     req.LockTtl,
			Kind:    kind,
